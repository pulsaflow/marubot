...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (queue.tracks.size < 2) {
      await interaction.editReply({
        embeds: [
          createErrorEmbed({
            title: '❌ Erreur',
            description: "Il faut au moins 2 musiques dans la file d'attente pour les mélanger.",
            guild: interaction.guild ?? undefined,
          }),
        ],
      });
    ...
    musicService.shuffle(queue);

    await interaction.editReply({
      embeds: [
        createSuccessEmbed({
          title: '🔀 Mélange',
          description: `Les ${queue.tracks.size} musiques de la file d'attente ont été mélangées.`,
          guild: interaction.guild ?? undefined,
        }),
      ],
    });
